...
async function callGemini(payload, model = GEMINI_PRIMARY_MODEL) {
...
/**
 * @desc    Analyze room photo or blueprint using Gemini Multimodal Vision
 * @route   POST /api/ai/analyze-room
 * @access  Public
 */
export const analyzeRoom = async (req, res) => {
  try {
    const { base64Data, mimeType = "image/jpeg", roomContext = "space photo or architectural blueprint" } = req.body;
...
Respond with a STRICT JSON OBJECT (no markdown ticks, no commentary outside JSON) in this exact schema:
{
  ...
  "suggestedItems": [
    ...
    console.warn("AI analyzeRoom falling back to architectural preset:", err.message);
...
/**
 * @desc    Enhance interior prompt with Gemini
...
Respond in STRICT JSON (no markdown ticks, only valid JSON):
{
  ...
  "designSummary": "A punchy 1-sentence design statement capturing the design essence."
    ...
    console.warn("AI enhancePrompt falling back:", err.message);
    const style = req.body.style || "Modern";
    const roomType = req.body.roomType || "Living Room";
    const dimensions = req.body.dimensions || "16ft × 14ft";
    res.json({
      success: true,
...
/**
 * @desc    Audit a 2D floor plan with Gemini
 * @route   POST /api/ai/audit-floorplan
 * @access  Public
 */
export const auditFloorPlan = async (req, res) => {
  try {
...
Provide a strict JSON response (no markdown, pure JSON):
{
  ...
  "vastuNotes": "Brief positive guidance on entrance and kitchen orientation"
    ...
    console.warn("AI auditFloorPlan falling back:", err.message);
    res.json({
      success: true,
      result: {
        overallScore: 92,
...
export default { analyzeRoom, enhancePrompt, auditFloorPlan };
